...
static int secp256k1_bppp_rangeproof_batch_add_impl(
    const secp256k1_context* ctx,
    secp256k1_scratch_space* scratch,
    const secp256k1_bppp_generators* gens,
    secp256k1_ge* asset_genp,
    ...
    /* Ecmult to compute C = S/T + M*delta + t*D + t^2R + 2t^3V + <g_vec_pub_deltas, G_vec> + v_g*A(asset_genP) */
    {
        ...
        res = secp256k1_bppp_rangeproof_norm_product_batch_add(
            ctx,
            scratch,
            &proof[33*4],
            proof_len - 33*4,
...
static int secp256k1_bppp_rangeproof_batch_verify_impl(
    ...
    res = secp256k1_bppp_rangeproof_norm_product_batch_verify(
        ctx,
        scratch,
        batch
    );
